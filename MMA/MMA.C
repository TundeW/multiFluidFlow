              ...
              std::vector<std::vector<double>> &dgdx)
{
    ...
    //std::cout << "Generate the subproblem";
    GenSub(xval, dfdx, g, dgdx);
    //Info<<"Subproblem Generated!"<<endl;
    std::cout << "Subproblem Generated!";
                  ...
                  std::vector<double> &g, std::vector<std::vector<double>> &dgdx)
{
    ...
    for (int i = 0; i < n; i++)
    {
    	//std::cout << "The For Loop" << i << " " << n;
        xmax[i] = std::min(10., xval[i] + movelimit);
        xmin[i] = std::max(-10., xval[i] - movelimit);
    }
    // Set asymptotes
    if (iter < 3)
    {
