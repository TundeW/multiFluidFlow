...
void tractionDisplacementFvPatchVectorField::updateCoeffs()
{
    ...
    const fvPatchField<scalar>& vf1 =
        patch().lookupPatchField<volScalarField, scalar>("vf1");
    ...
    scalarField mu(qd*(1-vf1)/(qd+vf1)*(E-Emin)/(2.0*(1.0+Po))+Emin/(2.0*(1.0+Po)));
    scalarField lambda(qd*(1-vf1)/(qd+vf1)*Po*(E-Emin)/((1.0+Po)*(1.0-2.0*Po))+Po*Emin/((1.0+Po)*(1.0-2.0*Po)));
    if (planeStress)
    {
        lambda=qd*(1-vf1)/(qd+vf1)*Po*(E-Emin)/((1.0+Po)*(1.0-Po))+Po*Emin/((1.0+Po)*(1.0-Po));
    }
